...
from Library.technical_analysis.trend.ema_trend_V2 import check_ema_trend
...
from Library.broker_interface.mt5_broker import cancel_expired_pending_orders, manage_trailing_stop, cancel_order_by_ticket
    ...
    def run(self):
        ...
        while self.is_running:
            for config in self.configs:
                ...
                analyzer = self.spread_analyzers[symbol]

                if self.symbol_error_counts.get(symbol, 0) > 5:
                    continue
                
                try:
                    manage_trailing_stop(symbol, config['magic_number'])
                    cancel_expired_pending_orders(config['magic_number'], 3600)
                ...
                analyzer.add_spread(current_spread)
                ...
                if market_status:
                    ...
                    if market_status.get("condition") == "Data Tidak Tersedia":
                        self.symbol_error_counts[symbol] += 1
                        if self.symbol_error_counts[symbol] == 1: # Log hanya pada error pertama
                             self.log_update.emit(symbol, "WARNING: Gagal mengambil data histori. Memeriksa kembali...")
                        elif self.symbol_error_counts[symbol] > 5:
                             self.log_update.emit(symbol, f"❌ ERROR: Gagal mengambil data untuk {symbol} 5 kali. Pair ini dihentikan sementara.")
                        continue
                    self.symbol_error_counts[symbol] = 0

                trade_signal_data = market_status.get("trade_signal") if market_status else None

                old_ticket = market_status.get("existing_pending_ticket")

                if old_ticket and trade_signal_data:
                    # Dapatkan info detail dari order lama untuk membandingkan harga
                    old_order_list = mt5.orders_get(ticket=old_ticket)
                    if old_order_list:
                        old_order = old_order_list[0]
                        old_price = old_order.price_open
                        new_price = trade_signal_data['entry']

                        # Tentukan toleransi, misalnya 5 points dari pair tersebut
                        # Point adalah unit terkecil perubahan harga (misal: 0.00001 untuk EURUSD)
                        tolerance = symbol_info.point * 5 
                        price_difference = abs(new_price - old_price)

                        # Jika perbedaan harga MASIH di dalam toleransi, anggap sinyal SAMA
                        if price_difference <= tolerance:
                            # Batalkan sinyal baru agar tidak terjadi penempatan order duplikat
                            trade_signal_data = None 
                            log_msg = (
                                f"Sinyal baru diabaikan, harga {new_price:.5f} terlalu mirip "
                                f"dengan order lama #{old_ticket} di harga {old_price:.5f}."
                            )
                            self.log_update.emit(symbol, log_msg)
                        else:
                            # Jika perbedaan signifikan, barulah batalkan order lama
                            is_cancelled, cancel_msg = cancel_order_by_ticket(old_ticket)
                            if is_cancelled:
                                self.log_update.emit(symbol, f"↪️ Pending order lama #{old_ticket} di harga {old_price:.5f} dibatalkan untuk diganti sinyal baru di {new_price:.5f}.")
                            else:
                                self.log_update.emit(symbol, f"⚠️ Gagal membatalkan order lama #{old_ticket}: {cancel_msg}. Sinyal baru dilewati.")
                                continue
                
                if trade_signal_data:
                    entry_price = trade_signal_data['entry']
                    rounded_price = round(entry_price, symbol_info.digits)
                    order_key = f"{symbol}_{rounded_price}"
                    current_time = time.time()
                    
                    if order_key in self.placed_orders_cache and (current_time - self.placed_orders_cache[order_key] < 60):
                        continue
                    ...
                    order_type = None
                    if signal_type_str == "BUY_LIMIT":
                        order_type = mt5.ORDER_TYPE_BUY_LIMIT
                    elif signal_type_str == "SELL_LIMIT":
                        order_type = mt5.ORDER_TYPE_SELL_LIMIT
                    # (Opsional) Tambahkan ini jika Anda ingin V2 bisa handle STOP order juga
                    elif signal_type_str == "BUY_STOP":
                        order_type = mt5.ORDER_TYPE_BUY_STOP
                    elif signal_type_str == "SELL_STOP":
                        order_type = mt5.ORDER_TYPE_SELL_STOP

                    # Jika order_type tidak terdefinisi, jangan lanjutkan
                    if order_type is None:
                        self.log_update.emit(symbol, f"Peringatan: Tipe sinyal tidak dikenal '{signal_type_str}'.")
                        continue
                    
                    lot_size, msg = calculate_lot_size(config['risk_per_trade'], entry_price, sl_level, symbol)
                    
                    if lot_size:
                        is_placed, exec_message = place_pending_order(symbol, order_type, lot_size, entry_price, sl_level, tp_level, magic_number)
                        if is_placed:
...
class RobotApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("7FX Automation Control Panel V2")
